	                                        ...
	                                        <table class="txt project_txt">
	                                            ...
	                                            <c:forEach var="item" items="${model.list}" varStatus="status">
		                                            <tr>
		                                                ...
		                                                <td>${fn:substring(item.START_TM,0,16)}<br> ~ <br>${fn:substring(item.END_TM,0,16)}</td>
		                                                <td>
		                                                	<a href="${pageContext.request.contextPath}/user/portfolio/detail/${item.MEMBER_ID}/${item.IDX}/view.do?MEMBER_ID=${model.MEMBER_ID}">
			                                                	<c:choose>
			                                                		<c:when test="${item.COURSE == 1}">찾아가는 영재교육 프로그램</c:when>
			                                                		<c:when test="${item.COURSE == 2}">체험진로탐색 프로그램</c:when>
			                                                		<c:when test="${item.COURSE == 3}">창의융합캠프</c:when>
		                                                ...
		                                                <td><a href="${pageContext.request.contextPath}/user/portfolio/detail/${item.MEMBER_ID}/${item.IDX}/view.do?MEMBER_ID=${model.MEMBER_ID}">${item.PRGM_NM}</a></td>
		                                                <td>${item.PLACE}</td>
		                                                <td>${item.RTICIPATION_TM}</td>
		                                                <td>${item.STFT}</td>
		                                                <td>
